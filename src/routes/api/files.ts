      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          if (contentType.includes('multipart/form-data')) {
            ...
            // Check destination
            let destinationBase = resolvedTarget
            try {
              const stat = await fs.stat(resolvedTarget)
              if (!stat.isDirectory()) {
                destinationBase = path.dirname(resolvedTarget)
              }
            } catch {
              destinationBase = path.dirname(resolvedTarget)
            }

            // Path Traversal Fix: Use path.basename() to strip directory separators from user-provided filename
            const fileName = path.basename(file.name)
            const destination = path.join(destinationBase, fileName)

            // Final check that we are still inside workspace
