...
async function getBuiltinRoot(): Promise<string | null> {
  ...
  try {
    ...
    const globalRoot = cp.execFileSync('npm', ['root', '-g'], { encoding: 'utf8', timeout: 3000, stdio: ['pipe', 'pipe', 'pipe'] }).trim()
...
async function ensureMarketplaceRegistry(): Promise<boolean> {
  ...
  try {
    ...
    cp.execFileSync(
      'git',
      ['clone', '--depth', '1', '--single-branch', REGISTRY_REPO, path.dirname(MARKETPLACE_ROOT)],
