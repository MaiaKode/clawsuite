...
function isGitAvailable(): boolean {
  ...
  try {
    execFileSync('git', ['--version'], { timeout: 5_000, stdio: 'pipe' })
    gitAvailable = true
  } catch {
    gitAvailable = false
  }
...
function runUpdate(): { ok: boolean; output: string } {
  ...
  try {
    ...
    const pullOutput = execFileSync('git', ['pull', remote, currentBranch], {
      cwd: repoPath,
      timeout: 30_000,
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'pipe'],
    ...
    // Install deps
    const installOutput = execFileSync('npm', ['install', '--prefer-offline'], {
      cwd: repoPath,
      timeout: 120_000,
      encoding: 'utf8',
    }).trim()
